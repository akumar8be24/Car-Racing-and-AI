...
from .lib.time import parse_time_string
from .lib.tyres import get_tyre_compound_int, get_tyre_compound_str

APP_NAME = "open-pit-wall"
APP_HOME_ENV_VAR = "OPEN_PIT_WALL_HOME"
FASTF1_CACHE_DIRNAME = ".fastf1-cache"
COMPUTED_DATA_DIRNAME = "computed_data"
CACHE_FILE_EXTENSION = ".json"
LEGACY_CACHE_FILE_EXTENSION = ".pkl"

def enable_cache():
    cache_path = get_fastf1_cache_directory()
...
SESSION_TYPE_DETAILS = {
    "R": {"cache_suffix": "race", "label": "Full Race"},
    "S": {"cache_suffix": "sprint", "label": "Sprint Race"},
    "Q": {"cache_suffix": "quali", "label": "Qualifying"},
    "SQ": {"cache_suffix": "sprintquali", "label": "Sprint Qualifying"},
}
REPLAY_SCHEMA_VERSION = 2
TRACK_STATUS_DETAILS = {
    "1": {"message": "TRACK CLEAR", "flag": "GREEN"},
    "2": {"message": "YELLOW FLAG", "flag": "YELLOW"},
    "4": {"message": "SAFETY CAR DEPLOYED", "flag": "YELLOW"},
    "5": {"message": "RED FLAG", "flag": "RED"},
    "6": {"message": "VIRTUAL SAFETY CAR DEPLOYED", "flag": "YELLOW"},
    "7": {"message": "VIRTUAL SAFETY CAR ENDING", "flag": "YELLOW"},
}
...
def _timestamp_from_elapsed(replay_start_utc, elapsed_seconds):
    if replay_start_utc is None:
        return ""
    return _format_utc_timestamp(
        replay_start_utc + timedelta(seconds=float(elapsed_seconds))
    )


def _is_current_replay_payload(payload):
    return (
        isinstance(payload, dict)
        and int(payload.get("schema_version", 0)) == REPLAY_SCHEMA_VERSION
        and isinstance(payload.get("frames"), list)
        and isinstance(payload.get("race_control_messages"), list)
    )
...
def get_app_data_directory():
    configured_path = os.environ.get(APP_HOME_ENV_VAR)
    if configured_path:
        app_data_dir = Path(configured_path).expanduser()
    elif os.name == "nt":
        app_data_root = Path(
            os.environ.get("APPDATA", str(Path.home() / "AppData" / "Roaming"))
        )
        app_data_dir = app_data_root / APP_NAME
    else:
...
def get_fastf1_cache_directory():
    cache_dir = get_app_data_directory() / FASTF1_CACHE_DIRNAME
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir


def _safe_upper_text(value):
    if value is None:
        return ""
    try:
...
def _current_lap_at_time(frames, frame_times, event_time):
    if not frames:
        return 0
    frame_index = min(bisect_left(frame_times, float(event_time)), len(frames) - 1)
    return int(frames[frame_index].get("lap", 0))


def _build_track_status_event(status, global_t_min):
    seconds = timedelta.total_seconds(status["Time"])
    event_time = round(seconds - global_t_min, 3)
    if event_time <= 0.0:
        return None

    status_code = str(status.get("Status", "")).strip()
    details = TRACK_STATUS_DETAILS.get(status_code, {})
    message = details.get("message", "TRACK STATUS UPDATED")
    flag = details.get("flag", "")
    raw_message = _safe_upper_text(status.get("Message"))
    if raw_message:
        message = raw_message.replace("ALLCLEAR", "TRACK CLEAR")
...
def _build_race_control_event(message, session, global_t_min):
    time_val = message["Time"]
    ...
    return {
        "time": event_time,
        "payload": {
            "message": _safe_upper_text(message.get("Message", "")),
            "flag": _safe_upper_text(message.get("Flag", "")),
            "scope": str(message.get("Scope", "") or "").strip(),
            "sector": _coerce_sector(message.get("Sector", 0)),
            "current_lap": 0,
        },
    }


def _build_driver_event_payload(driver_code, driver_data):
    tyre_name = get_tyre_compound_str(int(round(driver_data["tyre"])))
    return {
        "driver_code": driver_code,
        "current_lap_time": round(float(driver_data["current_lap_time"]), 3),
        "speed": round(float(driver_data["speed"]), 3),
        "rpm": int(round(driver_data["rpm"])),
        "throttle": round(float(driver_data["throttle"]), 3),
        "brake": round(float(driver_data["brake"]), 3),
        "gear": int(driver_data["gear"]),
        "drs_status": int(driver_data["drs"]),
        "current_tyre": tyre_name,
        "position": {
            "x": round(float(driver_data["x"]), 3),
            "y": round(float(driver_data["y"]), 3),
            "z": round(float(driver_data["z"]), 3),
            "dist_metres_around_track": round(float(driver_data["lap_dist"]), 3),
            "dist_percentage_around_track": round(float(driver_data["rel_dist"]), 6),
        },
    }


def _build_driver_telemetry_frames(frames, replay_start_utc):
    telemetry_frames = []
    for frame in frames:
        payload = [
            _build_driver_event_payload(driver_code, driver_data)
            for driver_code, driver_data in frame.get("drivers", {}).items()
        ]
        telemetry_frame = {
            "t": frame["t"],
            "lap": frame["lap"],
            "timestamp": _timestamp_from_elapsed(replay_start_utc, frame["t"]),
            "event": "telemetry.drivers",
            "payload": payload,
        }
        if "weather" in frame:
            telemetry_frame["weather"] = frame["weather"]
        if "safety_car" in frame:
            telemetry_frame["safety_car"] = frame["safety_car"]
        telemetry_frames.append(telemetry_frame)
    return telemetry_frames


def _build_merged_race_control_messages(raw_events, frames, replay_start_utc):
    frame_times = [float(frame["t"]) for frame in frames]
    merged_messages = []
    for event in sorted(raw_events, key=lambda item: item["time"]):
        payload = dict(event["payload"])
        payload["current_lap"] = _current_lap_at_time(
            frames, frame_times, event["time"]
        )
        merged_messages.append(
            {
                "timestamp": _timestamp_from_elapsed(replay_start_utc, event["time"]),
                "event": "race_control",
                "payload": payload,
            }
        )
...
def get_available_years(start_year=2018, end_year=None):
    """Return the available season years in descending order."""
    final_year = end_year or date.today().year
    return list(range(final_year, start_year - 1, -1))
...
def get_computed_data_directory():
    """Return the persistent directory used for saved computed telemetry."""
    computed_data_dir = get_app_data_directory() / COMPUTED_DATA_DIRNAME
    computed_data_dir.mkdir(parents=True, exist_ok=True)
    return computed_data_dir


def get_session_cache_suffix(session_type):
    """Return the cache suffix used for a given session type."""
    try:
        return SESSION_TYPE_DETAILS[session_type]["cache_suffix"]
    except KeyError as exc:
        raise ValueError(f"Unsupported session type: {session_type}") from exc


def get_session_cache_filename(year, round_number, event_name, session_type):
    """Return the file name for persisted telemetry data."""
    event_slug = _sanitize_cache_component(event_name)
    session_suffix = get_session_cache_suffix(session_type)
    return (
        f"{int(year)}_round_{int(round_number):02d}_{event_slug}_"
        f"{session_suffix}_telemetry{CACHE_FILE_EXTENSION}"
    )


def get_session_cache_candidates(year, round_number, event_name, session_type):
    """Return the possible on-disk locations for a telemetry cache file."""
    filename = get_session_cache_filename(year, round_number, event_name, session_type)
    preferred_path = get_computed_data_directory() / filename
    legacy_path = Path("computed_data") / filename
    if legacy_path == preferred_path:
        return [preferred_path]
    return [preferred_path, legacy_path]


def get_legacy_pickle_cache_candidates(year, round_number, event_name, session_type):
    filename = get_session_cache_filename(
        year, round_number, event_name, session_type
    ).replace(CACHE_FILE_EXTENSION, LEGACY_CACHE_FILE_EXTENSION)
    preferred_path = get_computed_data_directory() / filename
    legacy_path = Path("computed_data") / filename
    if legacy_path == preferred_path:
        return [preferred_path]
...
def find_session_cache_path(year, round_number, event_name, session_type):
    """Return the existing cache path for a session, if it exists."""
    for cache_path in get_session_cache_candidates(
        year, round_number, event_name, session_type
    ):
...
def is_session_data_downloaded(year, round_number, event_name, session_type):
    """Return whether telemetry for the requested session is already saved."""
    return (
        find_session_cache_path(year, round_number, event_name, session_type) is not None
    )


def load_cached_session_data(year, round_number, event_name, session_type):
    """Load persisted telemetry for a race or sprint session."""
    cache_path = find_session_cache_path(year, round_number, event_name, session_type)
    if cache_path is None:
        for legacy_pickle_path in get_legacy_pickle_cache_candidates(
            year, round_number, event_name, session_type
        ):
            if legacy_pickle_path.exists():
                raise ValueError(
                    "Legacy pickle cache files are no longer supported. "
                    f"Re-download {event_name} ({session_type}) to regenerate "
                    f"{legacy_pickle_path.name} as JSON."
                )
        raise FileNotFoundError(
            f"No saved telemetry found for {event_name} ({session_type})."
        )

    payload = _read_json_payload(cache_path)
    if not _is_current_replay_payload(payload):
        raise ValueError(
            f"Saved telemetry for {event_name} ({session_type}) is using an older schema."
        )
    return payload


def get_session_options_for_weekend(weekend):
    """Return the playable race session options for a weekend."""
    session_options = [{"code": "R", "label": SESSION_TYPE_DETAILS["R"]["label"]}]
    if str(weekend.get("type", "")).lower() in SPRINT_EVENT_FORMATS:
        session_options.insert(0, {"code": "S", "label": SESSION_TYPE_DETAILS["S"]["label"]})
    return session_options


def download_race_session(year, round_number, event_name, session_type, force_refresh=False):
    """Download and persist telemetry for a race or sprint session."""
    session = load_session(year, round_number, session_type)
    return get_race_telemetry(
        session,
...
def _process_single_driver(args):
    ...
    for _, lap in laps_driver.iterlaps():
        ...
        z_lap = (
            lap_tel["Z"].to_numpy()
            if "Z" in lap_tel
            else np.zeros_like(x_lap, dtype=float)
        )
        d_lap = lap_tel["Distance"].to_numpy()
        rd_lap = lap_tel["RelativeDistance"].to_numpy()
        current_lap_time_lap = t_lap - float(t_lap[0])
        speed_kph_lap = lap_tel["Speed"].to_numpy()
        rpm_lap = (
            lap_tel["RPM"].to_numpy()
            if "RPM" in lap_tel
            else np.zeros_like(speed_kph_lap, dtype=float)
        )
...
def get_race_telemetry(
    ...
    event = session.event
    resolved_year = year if year is not None else int(event["EventDate"].year)
    resolved_round = (
        round_number if round_number is not None else int(event["RoundNumber"])
    )
    resolved_event_name = event_name if event_name is not None else str(event["EventName"])
    cache_path = get_computed_data_directory() / get_session_cache_filename(
        resolved_year, resolved_round, resolved_event_name, session_type
    )
    cache_suffix = get_session_cache_suffix(session_type)
    ...
    try:
        if not force_refresh:
            payload = _read_json_payload(cache_path)
            if _is_current_replay_payload(payload):
                print(f"Loaded precomputed {cache_suffix} telemetry data.")
                print(f"Using saved data from {cache_path}.")
                return payload
            print(
                f"Saved {cache_suffix} telemetry data at {cache_path} uses an older schema; recomputing."
            )
    ...
    replay_start_utc = None
    session_start_utc = _coerce_utc_datetime(getattr(session, "t0_date", None))
    if session_start_utc is not None:
        replay_start_utc = session_start_utc + timedelta(seconds=float(global_t_min))
    ...
    for code, data in driver_data.items():
        ...
        resampled_data[code] = {
            ...
            "lap_dist": lap_dist_resampled,
            "rel_dist": rel_dist_resampled,
            "lap": lap_resampled,
            "current_lap_time": current_lap_time_resampled,
            "tyre": tyre_resampled,
    ...
    for status in track_status.to_dict("records"):
        ...
        track_status_event = _build_track_status_event(status, global_t_min)
        if track_status_event is not None:
            merged_race_control_events.append(track_status_event)
    ...
    if rc_messages is not None and not rc_messages.empty:
        for msg in rc_messages.to_dict("records"):
            race_control_event = _build_race_control_event(msg, session, global_t_min)
    ...
    for i in range(num_frames):
        ...
        for code in driver_codes:
            ...
            snapshot.append({
                "code": code,
                "dist": float(d["dist"][i]),
                "lap_dist": float(d["lap_dist"][i]),
                "x": float(d["x"][i]),
                "y": float(d["y"][i]),
                "z": float(d["z"][i]),
                "lap": int(round(d["lap"][i])),
                "current_lap_time": float(d["current_lap_time"][i]),
    ...
    telemetry_frames = _build_driver_telemetry_frames(frames, replay_start_utc)
    merged_race_control_messages = _build_merged_race_control_messages(
        merged_race_control_events, frames, replay_start_utc
    )
    ...
    replay_payload = {
        ...
        "replay_start_utc": _format_utc_timestamp(replay_start_utc),
        "frames": telemetry_frames,
        "driver_colors": get_driver_colors(session),
        "race_control_messages": merged_race_control_messages,
        "total_laps": int(max_lap_number),
        "max_tyre_life": max_tyre_life_map,
    }

    _write_json_payload(cache_path, replay_payload)

    print("Saved Successfully!")
    print(f"Saved telemetry to {cache_path}")
...
def get_quali_telemetry(
    ...
    try:
        if not force_refresh:
            data = _read_json_payload(cache_path)
            print(f"Loaded precomputed {cache_suffix} telemetry data.")
            print(f"Using saved data from {cache_path}.")
            return data
    except FileNotFoundError:
